...
import { blankSave, deepClone, format, player, saveSynergy, updateEffectiveLevelMult } from './Synergism'
...
export const reset = (input: resetNames, fast = false, from = 'unknown') => {
  ...
  player.multiplierBought = new Decimal('0')
  player.acceleratorCost = new Decimal('500')
  player.acceleratorBought = new Decimal('0')
  ...
  if (types.includes(input)) {
    ...
    player.acceleratorBoostBought = new Decimal('0')
  ...
  if (input === 'reincarnation' || input === 'reincarnationChallenge') {
    if (player.corruptions.used.deflation > 10 && player.platonicUpgrades[11] > 0) {
      player.prestigePoints = player.prestigePoints.add(player.reincarnationPoints)
    }
  }
